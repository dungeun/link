...
export function OptimizedLanguageProvider({
  children,
  initialLanguage = "ko",
  ...
  const t = useCallback(
    (key: string, fallback?: string): string => {
      ...
      if (currentLanguage !== "ko") {
        ...
        if (koTranslation) {
          logger.debug(
            `Translation missing for ${key} in ${currentLanguage}, using Korean`,
            {
              module: "OptimizedLanguageContext",
            },
          );
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.translated && data.translated.length > 0) {
            const translated =
              data.translated[0].translations[currentLanguage] || text;
            // 캐시 저장
...
// 모든 번역 키 export (디버깅용)
export const availableTranslationKeys = Object.keys(
  koTranslations,
) as TranslationKey[];
