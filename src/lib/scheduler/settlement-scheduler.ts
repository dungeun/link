...
try {
  ...
  console.warn(
    "node-cron이 설치되지 않았습니다. 자동 정산 스케줄러가 비활성화됩니다.",
  );
}
...
export class SettlementScheduler {
  private tasks: Map<
    string,
    ReturnType<typeof import("node-cron").schedule> | undefined
  > = new Map();
   ...
   * 정산 결과 로깅
   */
  private async logSettlementResult(
    type: string,
    result: { processed: number; failed: number; results?: unknown[] },
  ) {
    // 추후 DB에 로그 저장 구현
    // 현재는 콘솔 로그만
    const logData = {
