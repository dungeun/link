...
class EnvironmentConfig {
  ...
  private loadConfig(): void {
    this.config = {
      ...
      DATABASE_CONNECTION_LIMIT: parseInt(
        process.env.DATABASE_CONNECTION_LIMIT || "10",
      ),
      DATABASE_CONNECT_TIMEOUT: parseInt(
        process.env.DATABASE_CONNECT_TIMEOUT || "10",
      ),
      DATABASE_POOL_TIMEOUT: parseInt(
        process.env.DATABASE_POOL_TIMEOUT || "10000",
      ),
      DATABASE_IDLE_TIMEOUT: parseInt(
        process.env.DATABASE_IDLE_TIMEOUT || "10",
      ),
      SLOW_QUERY_THRESHOLD: parseInt(
  ...
  private validate(): void {
    ...
    if (missing.length > 0) {
      if (typeof window === "undefined" && process.env.VERCEL) {
        console.warn(
          `[Build Time] Missing environment variables: ${missing.join(", ")}`,
        );
