...
class ConfigManager {
  ...
  private validateProductionConfig() {
    // 프로덕션 필수 체크
    const requiredProdVars = ["ENCRYPTION_KEY", "TOSS_SECRET_KEY"];
    ...
    if (missing.length > 0) {
      throw new Error(
        `프로덕션 환경에서 필수 변수가 누락되었습니다: ${missing.join(", ")}`,
      );
    }
    ...
    if (this.config.JWT_SECRET === this.config.JWT_REFRESH_SECRET) {
      throw new Error(
        "JWT_SECRET과 JWT_REFRESH_SECRET은 다른 값이어야 합니다.",
      );
    }
  }
  ...
  public get security() {
    return {
      ...
      allowedOrigins:
        this.config.ALLOWED_ORIGINS?.split(",").map((o) => o.trim()) || [],
    };
  }
