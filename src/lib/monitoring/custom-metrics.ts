...
export class BusinessDashboard extends EventEmitter {
  ...
  private sendKPIsToDataDog(kpis: Record<string, any>): void {
    ...
    metrics.gauge(
      "business.campaigns.budget.total",
      kpis.campaigns.totalBudget,
    );
    metrics.gauge(
      "business.campaigns.conversion_rate",
      kpis.campaigns.conversionRate,
    );

    // 사용자 메트릭
    metrics.gauge(
      "business.users.total",
      kpis.users.totalBusinesses + kpis.users.totalInfluencers,
    );
    ...
    // 성능 메트릭
    metrics.gauge(
      "business.performance.response_time",
      kpis.performance.avgResponseTime,
    );
    metrics.gauge(
      "business.performance.error_rate",
      kpis.performance.errorRate,
    );
...
export class UserBehaviorAnalytics {
  ...
  static trackAction(
    ...
    metrics.increment(`user.action.${action}`, 1, [`category:${category}`]);
...
export class AlertingSystem {
  ...
  static async sendAlert(alert: {
    ...
    if (alert.level === "critical") {
      ErrorBoundary.captureMessage(alert.title, "error", {
        ...alert.metadata,
        metric: alert.metric,
        value: alert.value,
