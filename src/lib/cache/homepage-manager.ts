...
const HOMEPAGE_JSON_PATH = path.join(process.cwd(), 'public/cache/homepage-unified.json')
...
export class HomepageManager {
  ...
  async syncFromDatabase(): Promise<boolean> {
    try {
      ...
      if (mainPageConfig.heroSlides && homepage.sections.hero) {
        const updatedSlides = await Promise.all(
          mainPageConfig.heroSlides.map(async (slide: any) => ({
            ...
            tag: slide.tag ? await this.generateMultiLanguageText(slide.tag) : null,
      ...
      if (mainPageConfig.categoryMenus && homepage.sections.category) {
        const updatedCategories = await Promise.all(
          mainPageConfig.categoryMenus.map(async (cat: any) => ({
            ...
            slug: cat.categoryId,
            icon: cat.icon,
            badge: cat.badge ? await this.generateMultiLanguageText(`category.badge.${cat.badge}`) : null,
      ...
      if (mainPageConfig.promoBanner && homepage.sections.promo) {
        const updatedPromo = {
          title: await this.generateMultiLanguageText(mainPageConfig.promoBanner.title),
          subtitle: await this.generateMultiLanguageText(mainPageConfig.promoBanner.subtitle),
          ...
          backgroundColor: mainPageConfig.promoBanner.backgroundColor || "#FEF3C7",
      ...
      // 5. Section Order 동기화
      if (mainPageConfig.sectionOrder && Array.isArray(mainPageConfig.sectionOrder)) {
        const newOrder = mainPageConfig.sectionOrder.map((section: any) => section.id || section.type)
      ...
      logger.error(`Database sync failed: ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
   ...
   */
  private async generateMultiLanguageText(languageKey: string): Promise<{ko: string, en: string, jp: string}> {
    try {
      ...
      // 각 언어에서 해당 키의 번역 찾기
      const ko = this.findTranslation(languagePacks, languageKey, 'ko') || languageKey
      const en = this.findTranslation(languagePacks, languageKey, 'en') || `[EN] ${ko}`
      const jp = this.findTranslation(languagePacks, languageKey, 'jp') || `[JP] ${ko}`
      
      return { ko, en, jp }
    } catch (error) {
      logger.error(`Failed to generate multi-language text: ${error instanceof Error ? error.message : String(error)}`)
   ...
   * 언어팩에서 번역 찾기
   */
  private findTranslation(languagePacks: any[], key: string, language: string): string | null {
  ...
  private extractFallbackText(languageKey: string): string {
    const fallbackMap: Record<string, string> = {
      ...
      'hero.slide6.subtitle': '믿을 수 있는 에스크로 결제로 안전하게 거래하세요',
    ...
    return fallbackMap[languageKey] || languageKey.split('.').pop() || languageKey
  }
  ...
  async loadHomepage(): Promise<HomepageData | null> {
    try {
      ...
      logger.error(`Failed to load homepage data: ${error instanceof Error ? error.message : String(error)}`)
  ...
  async updateSection(updateData: SectionUpdateData): Promise<boolean> {
    try {
      ...
      logger.error(`Failed to update section: ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
   ...
   */
  async generateSectionTranslation(sectionId: string, sourceData: any, targetLanguage: 'en' | 'jp'): Promise<any> {
    try {
      // Google Translate API 호출하여 번역 생성
      const translatedData = await this.translateSectionTexts(sourceData, 'ko', targetLanguage)
      
      logger.info(`Translation generated for section ${sectionId} to ${targetLanguage}`)
      return translatedData
    } catch (error) {
      logger.error(`Failed to generate translation: ${error instanceof Error ? error.message : String(error)}`)
      return null
    }
  }
   ...
   * 섹션 텍스트 자동 번역
   */
  private async translateSectionTexts(data: any, from: string, to: string): Promise<any> {
  ...
  private async callTranslateAPI(text: string, targetLang: string): Promise<string> {
    try {
      ...
      if (!googleTranslateApiKey) {
        logger.warn('Google Translate API key not found, using fallback translation')
      ...
      if (translatedText) {
        logger.info(`Translated "${text}" to ${targetLang}: "${translatedText}"`)
        return translatedText
      } else {
        throw new Error('No translation returned from API')
      }
    } catch (error) {
      logger.error(`Translation failed for "${text}" to ${targetLang}: ${error instanceof Error ? error.message : String(error)}`)
      return this.getFallbackTranslation(text, targetLang)
    }
  }
  ...
  private getFallbackTranslation(text: string, targetLang: string): string {
    ...
    return fallbackTranslations[text]?.[targetLang] || `[AUTO-${targetLang.toUpperCase()}] ${text}`
  }
  ...
  async updateSectionOrder(newOrder: string[]): Promise<boolean> {
    try {
      ...
      logger.error(`Failed to update section order: ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
  ...
  async toggleSectionVisibility(sectionId: string): Promise<boolean> {
    try {
      ...
      // 가시성 토글
      homepage.sections[sectionId].visible = !homepage.sections[sectionId].visible
      homepage.metadata.lastUpdated = new Date().toISOString()
      ...
      logger.error(`Failed to toggle section visibility: ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
  ...
  async getSectionsByLanguage(language: LanguageCode): Promise<any[]> {
    try {
      ...
      logger.error(`Failed to get sections by language: ${error instanceof Error ? error.message : String(error)}`)
      return []
    }
  }
  ...
  private async restoreFromBackup(): Promise<HomepageData | null> {
    try {
      ...
      const backupFiles = files
        .filter(file => file.startsWith('homepage-') && file.endsWith('.json'))
        .sort()
        .reverse()
      ...
      logger.error(`Failed to restore from backup: ${error instanceof Error ? error.message : String(error)}`)
      return null
    }
  }
  ...
  async checkAndApplyOrderChanges(): Promise<boolean> {
    try {
      ...
      logger.error(`Failed to check order changes: ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
