...
export class RedisCacheLayer {
    ...
    try {
      ...
      if (cached) {
        ...
        if ((this.hitCount + this.missCount) % 1000 === 0) {
          logger.info(`Cache stats - Hit rate: ${hitRate.toFixed(2)}%, Hits: ${this.hitCount}, Misses: ${this.missCount}`);
        }
   ...
   * 패턴별 캐시 무효화
   */
  async invalidateByType(type: 'campaigns' | 'users' | 'categories' | 'all'): Promise<void> {
    const patterns = {
