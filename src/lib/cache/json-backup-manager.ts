...
export class JsonBackupManager {
  ...
  async saveWithBackup(
    ...
    try {
      ...
      if (defaultExists) {
        const backup1Path = path.join(this.basePath, `${fileNameWithoutExt}-backup-1${fileExt}`)
      ...
      logger.info(`Saved new data with backup: ${fileName} (${jsonData.metadata.size} bytes)`)

      return true
    } catch (error) {
      logger.error(`Failed to save with backup: ${fileName} - ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
   ...
   * 백업 파일들을 한 단계씩 뒤로 회전
   */
  private async rotateBackups(fileNameWithoutExt: string, fileExt: string): Promise<void> {
    // backup-4 삭제 (가장 오래된 백업)
    const backup4Path = path.join(this.basePath, `${fileNameWithoutExt}-backup-4${fileExt}`)
    if (await this.fileExists(backup4Path)) {
      await fs.unlink(backup4Path)
      logger.info(`Deleted oldest backup: ${fileNameWithoutExt}-backup-4${fileExt}`)
    }

    // backup-3 → backup-4, backup-2 → backup-3, backup-1 → backup-2
    for (let i = 3; i >= 1; i--) {
      const currentPath = path.join(this.basePath, `${fileNameWithoutExt}-backup-${i}${fileExt}`)
      const nextPath = path.join(this.basePath, `${fileNameWithoutExt}-backup-${i + 1}${fileExt}`)
  ...
  async restoreFromBackup(
    ...
    try {
      const fileNameWithoutExt = path.parse(fileName).name
      const fileExt = path.parse(fileName).ext
      const backupPath = path.join(this.basePath, `${fileNameWithoutExt}-backup-${backupLevel}${fileExt}`)
      ...
      logger.error(`Failed to restore from backup: ${fileName} level ${backupLevel} - ${error instanceof Error ? error.message : String(error)}`)
      return null
    }
  }
  ...
  async listBackups(fileName: string): Promise<BackupMetadata[]> {
    try {
      ...
      for (let i = 1; i <= this.options.maxBackups; i++) {
        const backupPath = path.join(this.basePath, `${fileNameWithoutExt}-backup-${i}${fileExt}`)
        if (await this.fileExists(backupPath)) {
      ...
      logger.error(`Failed to list backups: ${fileName} - ${error instanceof Error ? error.message : String(error)}`)
      return []
    }
  }
  ...
  async loadData(fileName: string): Promise<any | null> {
    try {
      ...
      logger.error(`Failed to load data: ${fileName} - ${error instanceof Error ? error.message : String(error)}`)
   ...
   * 백업 무결성 검증
   */
  async verifyBackup(fileName: string, backupLevel: number = 0): Promise<boolean> {
    try {
      const fileNameWithoutExt = path.parse(fileName).name
      const fileExt = path.parse(fileName).ext
      
      const targetPath = backupLevel === 0 
        ? path.join(this.basePath, fileName)
        : path.join(this.basePath, `${fileNameWithoutExt}-backup-${backupLevel}${fileExt}`)
      ...
      logger.error(`Backup verification failed: ${fileName} level ${backupLevel} - ${error instanceof Error ? error.message : String(error)}`)
      return false
    }
  }
