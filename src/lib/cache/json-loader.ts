 ...
 */
async function loadJsonFile(filePath: string, fallbackPaths: string[] = []): Promise<JsonCacheData | null> {
  ...
  for (const currentPath of allPaths) {
    try {
      ...
      logger.warn(`Failed to load JSON file: ${currentPath} - ${error instanceof Error ? error.message : String(error)}`)
      continue
    }
  }
 ...
 * 정적 UI 텍스트 로드
 */
export async function loadStaticUITexts(language: LanguageCode = 'ko'): Promise<StaticUITexts | null> {
...
export async function loadHomepageData(): Promise<HomepageData | null> {
  ...
  try {
    ...
    const [sectionsData, heroData, categoriesData] = await Promise.all([
      loadJsonFile(
        path.join(basePath, 'sections.json'),
        [
 ...
 */
export async function updateJsonFile(filePath: string, data: any): Promise<boolean> {
  try {
    ...
    logger.error(`Failed to update JSON file: ${filePath} - ${error instanceof Error ? error.message : String(error)}`)
    return false
  }
}
