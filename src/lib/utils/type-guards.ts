...
export function isApiError(value: unknown): value is ApiError {
  ...
  return (
    'code' in value && isString(value.code) &&
...
export function isStandardCampaign(value: unknown): value is StandardCampaign {
  ...
  if ('budget' in campaign && campaign.budget !== null) {
    ...
    if (!('amount' in campaign.budget) || !isNumber(campaign.budget.amount)) return false
  }
...
// 캠페인 응답 검증
export function validateCampaignResponse(data: unknown): StandardCampaign | null {
  try {
...
export function isUserInteractions(value: unknown): value is UserInteractions {
  ...
  if ('hasApplied' in interactions && !isBoolean(interactions.hasApplied)) return false
  if ('applicationStatus' in interactions && !isString(interactions.applicationStatus)) return false
  if ('isLiked' in interactions && !isBoolean(interactions.isLiked)) return false
  if ('isSaved' in interactions && !isBoolean(interactions.isSaved)) return false
...
export function isPaginationInfo(value: unknown): boolean {
  ...
  return (
    'page' in pagination && isNumber(pagination.page) &&
    'limit' in pagination && isNumber(pagination.limit) &&
    'total' in pagination && isNumber(pagination.total) &&
    'totalPages' in pagination && isNumber(pagination.totalPages)
  )
}