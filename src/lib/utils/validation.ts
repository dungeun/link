...
export class ValidationHelper {
  ...
  static validateImageFile(
    ...
    if (!this.validateFileType(file, allowedTypes)) {
      errors.push(
        "지원되지 않는 이미지 형식입니다. (JPEG, PNG, GIF, WebP만 허용)",
      );
    }
  ...
  static validateDocumentFile(
    ...
    if (!this.validateFileType(file, allowedTypes)) {
      errors.push(
        "지원되지 않는 문서 형식입니다. (PDF, DOC, DOCX, TXT만 허용)",
      );
    }
