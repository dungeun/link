...
export function getTranslatedCampaignData(
  ...
  if (
    ...
    (campaign.campaignTranslations as any).length > 0
  ) {
    const translation = (
      campaign.campaignTranslations as Array<{
        language: string;
        title?: string;
        description?: string;
        requirements?: string;
  ...
  if (campaign.translations) {
    try {
      const translations =
        typeof campaign.translations === "string"
      ...
      if (translations[`title_${language}`]) {
        return {
          ...
          description:
            translations[`description_${language}`] || campaign.description,
          requirements:
            translations[`requirements_${language}`] || campaign.requirements,
...
export function getTranslatedPostData(
  ...
  if (post.postTranslations && (post.postTranslations as any).length > 0) {
    const translation = (
      post.postTranslations as Array<{
        language: string;
        title?: string;
        content?: string;
      }>
