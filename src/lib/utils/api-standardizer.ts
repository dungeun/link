...
export function standardizePlatforms(platforms: string[] | string): Platform[] {
  ...
  return platformArray.map((platform) => {
    ...
    return (
      PLATFORM_MAPPING[upperPlatform] || {
...
// 미디어 파일 정보 표준화
export function standardizeMediaFile(
  url: string,
  filename?: string,
): MediaFile | undefined {
...
export function standardizeCampaign(campaign: any): StandardCampaign {
  ...
  const primaryCategory =
    categories.find((cat) => cat.isPrimary) || categories[0];
  ...
  return {
    ...
    content: {
      ...
      keywords: campaign.keywords
        ? campaign.keywords.split(",").map((k: string) => k.trim())
        : [],
    ...
    media: {
      thumbnail: standardizeMediaFile(
        campaign.thumbnailImageUrl || campaign.imageUrl,
      ),
...
export const createStandardError = {
  ...
  unauthorized: () =>
    createErrorResponse(API_ERROR_CODES.UNAUTHORIZED, "인증이 필요합니다."),
