    ...
    try {
      ...
      for (const url of urls) {
        try {
          ...
          const subscriberRegex =
            /"subscriberCountText":\s*\{\s*(?:"simpleText":\s*"([^"]+)"|"runs":\s*\[\s*\{\s*"text":\s*"([^"]+)")/g;
          let match;
          ...
          while ((match = subscriberRegex.exec(html)) !== null) {
            ...
            if (text && text.toLowerCase().includes("subscriber")) {
              ...
              if (numberMatch) {
                ...
                console.log(
                  `[NEW] YouTube: Extracted subscribers: ${subscribers} from "${text}"`,
                );
                break;
              }
            }
          ...
          if (subscribers === 0) {
            ...
            for (const pattern of directPatterns) {
              ...
              if (directMatch) {
                ...
                console.log(
                  `[NEW] YouTube: Found direct subscriber count: ${subscribers}`,
                );
                break;
              }
            }
          ...
          if (subscribers === 0) {
            ...
            for (const pattern of koreanPatterns) {
              ...
              if (koreanMatch) {
                ...
                console.log(
                  `[NEW] YouTube: Found Korean subscriber count: ${subscribers}`,
                );
                break;
              }
            }
                ...
                for (const match of matches) {
                  ...
                  if (numberMatch) {
                    ...
                    if (number > 0) {
                      subscribers = number;
                      console.log(
                        `[NEW] YouTube: Found subscribers with pattern: ${subscribers} from "${match}"`,
                      );
                      break;
                    }
                  }
          ...
          if (subscribers > 0) {
            console.log(
              `[NEW] YouTube: SUCCESS - Found ${subscribers} subscribers for ${searchName}`,
            );
            return {
          ...
          console.log(
            `[NEW] YouTube: Failed ${url}: ${e instanceof Error ? e.message : "Unknown error"}`,
          );
        }
      }
