  ...
  private async ensureUploadDir() {
    try {
      ...
      if (process.env.VERCEL) {
        console.log(
          "Vercel environment detected, files will be processed in memory only",
        );
      }
    }
  }
    ...
    try {
      ...
      if (process.env.VERCEL || process.env.NODE_ENV === "production") {
        ...
        try {
          ...
          throw new Error(
            `Vercel Blob Storage 업로드 실패: ${blobError instanceof Error ? blobError.message : "알 수 없는 오류"}`,
          );
        }
      }
      ...
      throw new Error(
        `파일 업로드에 실패했습니다: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
