...
export class UISectionService {
  // 모든 표시 가능한 섹션 가져오기
  static async getVisibleSections(
    language: string = "ko",
  ): Promise<UISection[]> {
    try {
          ...
          return {
            ...
            title: section.title
              ? isJsonObject(section.title)
                ? (section.title as unknown as Record<LanguageCode, string>)
                : undefined
              : undefined,
            subtitle: section.subtitle
              ? isJsonObject(section.subtitle)
                ? (section.subtitle as unknown as Record<LanguageCode, string>)
                : undefined
              : undefined,
            ...
            order: section.order,
            settings:
              section.settings && isJsonObject(section.settings)
                ? section.settings
                : undefined,
          };
        }
      ...
      logger.error(
        "Error fetching visible sections:",
  ...
  static async updateSectionOrder(
    orders: { sectionId: string; order: number }[],
  ): Promise<boolean> {
    try {
  ...
  static async toggleSectionVisibility(
    ...
    try {
      ...
      logger.error(
        `Error toggling section visibility ${sectionId}:`,
        error instanceof Error ? error.message : "Unknown error",
      );
      return null;
    }
