...
export default function CampaignsPage() {
  ...
  const categories = [
    ...
    {
      id: "패션",
      name: t("campaigns.category.fashion", "패션"),
      count: categoryStats["패션"] || 0,
    },
    {
      id: "뷰티",
      name: t("campaigns.category.beauty", "뷰티"),
      count: categoryStats["뷰티"] || 0,
    },
    {
      id: "음식",
      name: t("campaigns.category.food", "음식"),
      count: categoryStats["음식"] || 0,
    },
    {
      id: "여행",
      name: t("campaigns.category.travel", "여행"),
      count: categoryStats["여행"] || 0,
    },
    {
      id: "기술",
      name: t("campaigns.category.tech", "기술"),
      count: categoryStats["기술"] || 0,
    },
    {
      id: "라이프스타일",
      name: t("campaigns.category.lifestyle", "라이프스타일"),
      count: categoryStats["라이프스타일"] || 0,
    },
    {
      id: "스포츠",
      name: t("campaigns.category.sports", "스포츠"),
      count: categoryStats["스포츠"] || 0,
    },
    {
      id: "게임",
      name: t("campaigns.category.game", "게임"),
      count: categoryStats["게임"] || 0,
    },
    {
      id: "교육",
      name: t("campaigns.category.education", "교육"),
      count: categoryStats["교육"] || 0,
    },
    {
      id: "헬스",
      name: t("campaigns.category.health", "헬스"),
      count: categoryStats["헬스"] || 0,
    },
  ];
  ...
  const fetchCampaigns = useCallback(
    async (isInitial = false) => {
      ...
      try {
        ...
        logger.info(
          `Campaigns loaded: count=${data.items?.length || 0}, cursor=${data.nextCursor}, hasMore=${data.hasMore}`,
        );
      } catch (err) {
  ...
  const fetchLikedCampaigns = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const likedCampaignIds = data.campaigns.map(
          (c: { id: string }) => c.id,
        );
        ...
        <div className="container mx-auto px-6">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <option value="all">
                  {t("campaigns.filter.all_platforms", "모든 플랫폼")}
                </option>
              ...
              <div className="flex items-center gap-3 ml-auto">
                ...
                >
                  <option value="latest">
                    {t("campaigns.sort.latest", "최신순")}
                  </option>
                  <option value="deadline">
                    {t("campaigns.sort.deadline", "마감임박순")}
                  </option>
                  <option value="popular">
                    {t("campaigns.sort.popular", "인기순")}
                  </option>
                </select>
              </div>
            </div>
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="font-medium">
                    {t("error.data_loading_failed_title", "데이터 로딩 실패")}
                  </h3>
            ...
            <>
              <div className="mb-8">
                <p className="text-gray-600">
                  {t(
                    "campaigns.status.total_campaigns_count",
                    "총 {count}개의 캠페인이 있습니다.",
                  ).replace("{count}", `${totalCount || campaigns.length}`)}
                </p>
              </div>
              ...
              {campaigns.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <h3 className="mt-2 text-sm font-medium text-gray-900">
                    {t(
                      "campaigns.status.no_campaigns_available",
                      "캠페인이 없습니다",
                    )}
                  </h3>
                  <p className="mt-1 text-sm text-gray-500">
                    {t(
                      "campaigns.status.no_matching_campaigns",
                ...
                <>
                              ...
                              <div className="flex items-center justify-between pt-1">
                                ...
                                <p className="text-xs text-gray-500">
                                  {t(
                                    ...
                                    "{count}",
                                    campaign.applicant_count.toString(),
                                  )}
                                </p>
                              </div>
                  ...
                  {!hasMore && campaigns.length > 0 && (
                    <div className="text-center py-8 text-gray-500">
                      <p>
                        {t(
                          "campaigns.status.all_campaigns_loaded",
                          "모든 캠페인을 불러왔습니다.",
                        )}
                      </p>
                    </div>
      ...
      <section className="py-16 bg-gradient-to-r from-blue-600 to-cyan-600">
        <div className="container mx-auto px-6 text-center">
          <h2 className="text-3xl font-bold text-white mb-4">
            {t(
              "campaigns.cta.not_found_title",
              "원하는 캠페인을 찾지 못하셨나요?",
            )}
          </h2>
          <p className="text-xl text-white/80 mb-8">
            {t(
              "campaigns.cta.profile_register_desc",
