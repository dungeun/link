    ...
    try {
      ...
      if (response.ok) {
        ...
        const notices = data.posts
          .filter((post: Post) => post.category === "notice" && post.isPinned)
          .slice(0, 3);
        const regularPosts = data.posts.filter(
          (post: Post) => !(post.category === "notice" && post.isPinned),
        );
              ...
              {popularPosts.map((post, index) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span
                      className={`px-2 py-1 text-xs rounded ${getCategoryStyle(post.category)}`}
                    >
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <span
                            className={`px-2 py-1 text-xs rounded flex-shrink-0 ${getCategoryStyle(post.category)}`}
                          >
                            {getCategoryName(post.category)}
                          </span>
                          <Link
