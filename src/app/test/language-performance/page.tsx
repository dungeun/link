...
import { OptimizedLanguageProvider, useLanguage as useOptimizedLanguage } from '@/contexts/OptimizedLanguageContext';
import { LanguageProvider, useLanguage as useOriginalLanguage } from '@/contexts/LanguageContext';
  ...
  return (
    <div className="bg-green-50 p-6 rounded-lg">
      ...
      <div className="space-y-2 mb-4">
        <p>현재 언어: <span className="font-bold">{currentLanguage}</span></p>
        <p>전환 시간: <span className="font-bold text-green-600">{renderTime.toFixed(2)}ms</span></p>
...
function PerformanceTestOriginal() {
  ...
  return (
    <div className="bg-red-50 p-6 rounded-lg">
      ...
      <div className="space-y-2 mb-4">
        <p>현재 언어: <span className="font-bold">{currentLanguage}</span></p>
        <p>전환 시간: <span className="font-bold text-red-600">{renderTime.toFixed(2)}ms</span></p>
        <p>전환 횟수: {switchCount}</p>
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="mb-6 bg-blue-50 p-4 rounded-lg">
        ...
        <ul className="space-y-1 text-sm">
          <li>✅ <strong>정적 JSON:</strong> 언어 전환 즉시 (0ms), 모든 언어 메모리에 로드</li>
          <li>❌ <strong>API 호출:</strong> 매번 네트워크 요청 (200-500ms), 서버 부하 발생</li>
          <li>💾 <strong>메모리 사용:</strong> 102개 키 × 3개 언어 = 약 30KB (무시할 수준)</li>
          <li>🚀 <strong>예상 개선:</strong> 언어 전환 시 95% 이상 속도 향상</li>
        </ul>
      </div>
