...
function MyPageContent() {
  ...
  const { data: withdrawalsData, isLoading: withdrawalsLoading } =
    useInfluencerWithdrawals();
  const { data: savedCampaignsData, isLoading: loadingSavedCampaigns } =
    useSavedCampaigns();
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
          <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
            ...
            <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">
              {t("dashboard.stats.total_campaigns_desc", "총 참여한 캠페인 수")}
            </p>
          </div>
                        ...
                        <div className="space-y-2">
                          ...
                          {application.campaign?.businessName && (
                            <div className="flex items-center text-sm text-gray-600">
                              ...
                              <span>
                                브랜드: {application.campaign.businessName}
                              </span>
                            </div>
                          )}
                          {application.campaign?.reward && (
                            <div className="flex items-center text-sm text-gray-600">
                              ...
                              <span>
                                보상: ₩
                                {application.campaign.reward.toLocaleString()}
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="space-y-2 mb-4">
                        {campaign.reward && (
                          <div className="flex items-center text-sm text-gray-600">
                            <DollarSign className="w-4 h-4 mr-2" />
                            <span>
                              보상: ₩{campaign.reward.toLocaleString()}
                            </span>
                          </div>
                        )}
                        {campaign.endDate && (
            ...
            {profileSubTab === "sns" && (
              <SNSIntegrationSettings
                ...
                onPlatformUpdate={(platforms) =>
                  setConnectedPlatforms(platforms as any)
                }
                userId={user?.id || ""}
