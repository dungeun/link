  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        if (!token) {
          log.error('토큰이 없습니다.', 'AdminDashboard', {
        ...
        log.debug('토큰 발견됨', 'AdminDashboard', { tokenLength: token.length });
        ...
        if (!response.ok) {
          ...
          log.error('API 응답 에러', 'AdminDashboard', {
        ...
        log.exception('대시보드 데이터 로드 실패', error as Error, 'AdminDashboard')
        // 에러 시 기본값 설정
        setStats({
          totalUsers: 0,
          activeUsers: 0,
