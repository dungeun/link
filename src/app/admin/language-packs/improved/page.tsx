...
export default function ImprovedLanguagePacksPage() {
  ...
  const categoryStructure = {
    campaigns: {
      '병원': ['hospital.name', 'hospital.description', 'hospital.services', 'hospital.booking'],
      '캠페인': ['campaign.title', 'campaign.description', 'campaign.deadline', 'campaign.budget', 'campaign.applicants'],
      '구매평': ['review.title', 'review.content', 'review.rating', 'review.verified', 'review.helpful']
    },
    posts: {
      '글 작성': ['post.create', 'post.edit', 'post.save', 'post.publish'],
      '댓글': ['comment.write', 'comment.reply', 'comment.edit', 'comment.delete'],
      '리뷰': ['review.write', 'review.edit', 'review.submit', 'review.rating']
    },
    menus: {
      '헤더 메뉴': ['menu.home', 'menu.campaigns', 'menu.mypage', 'menu.login', 'menu.signup'],
      '푸터 메뉴': ['footer.about', 'footer.terms', 'footer.privacy', 'footer.contact'],
      '사이드바': ['sidebar.dashboard', 'sidebar.profile', 'sidebar.settings', 'sidebar.logout']
    },
    sections: {
  ...
  useEffect(() => {
    loadLanguagePacks()
    // 모든 카테고리 자동 확장
    const defaultExpanded = new Set(Object.keys(categoryStructure[activeTab] || {}))
  ...
  const loadLanguagePacks = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/admin/language-packs?category=${activeTab}`)
      if (response.ok) {
  ...
  const renderLanguagePackItem = (pack: LanguagePack) => {
    ...
    return (
      <div key={pack.id} className="bg-white border rounded-lg p-4 hover:shadow-md transition-shadow">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-gray-600">{pack.key}</span>
            {status === 'saved' && <Check className="w-4 h-4 text-green-500" />}
            {status === 'error' && <X className="w-4 h-4 text-red-500" />}
            {status === 'saving' && <RefreshCw className="w-4 h-4 text-blue-500 animate-spin" />}
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              <input
                ...
                onChange={(e) => {
                  setLanguagePacks(prev => 
                    prev.map(p => p.id === pack.id ? { ...p, ko: e.target.value } : p)
                  )
                }}
          ...
          <div>
              ...
              {isEditing && (
                <button
                  onClick={async () => {
                    ...
                    setLanguagePacks(prev => 
                      prev.map(p => p.id === pack.id ? { ...p, en: translated } : p)
                    )
                  }}
            ...
            {isEditing ? (
              <input
                ...
                onChange={(e) => {
                  setLanguagePacks(prev => 
                    prev.map(p => p.id === pack.id ? { ...p, en: e.target.value } : p)
                  )
                }}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
          ...
          <div>
              ...
              {isEditing && (
                <button
                  onClick={async () => {
                    ...
                    setLanguagePacks(prev => 
                      prev.map(p => p.id === pack.id ? { ...p, ja: translated } : p)
                    )
                  }}
                  className="text-xs text-blue-500 hover:text-blue-700"
                >
            ...
            {isEditing ? (
              <input
                ...
                onChange={(e) => {
                  setLanguagePacks(prev => 
                    prev.map(p => p.id === pack.id ? { ...p, ja: e.target.value } : p)
                  )
                }}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">언어팩 관리 (개선 버전)</h1>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800">
            <div className="space-y-2">
              <p>• <strong>탭별 관리:</strong> 캠페인, 게시물, 메뉴, 섹션별 분류</p>
              <p>• <strong>개별 편집:</strong> 각 언어팩 항목별 개별 수정</p>
              <p>• <strong>자동 번역:</strong> Google Translate API 개별 적용</p>
            </div>
            <div className="space-y-2">
              <p>• <strong>실시간 저장:</strong> 수정 즉시 개별 저장</p>
              <p>• <strong>상태 표시:</strong> 저장/오류 상태 실시간 표시</p>
              <p>• <strong>카테고리 접기:</strong> 대량 데이터 효율적 관리</p>
            </div>
          </div>
        </div>
      </div>
