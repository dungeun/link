    ...
    try {
      ...
      if (response.ok) {
        ...
        setStats({
          ...
          translatedKo: packs.filter((p: Record<string, unknown>) => p.ko)
            .length,
          translatedEn: packs.filter((p: Record<string, unknown>) => p.en)
            .length,
          translatedJp: packs.filter((p: Record<string, unknown>) => p.jp)
            .length,
        });
      }
    } catch (error) {
            ...
            >
              <div className="p-6">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-500">{category.count}개 항목</span>
              ...
              <div
                className={`h-1 ${getColorClasses(category.color).split(" ")[0]}`}
              />
            </div>
          ))}
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800">
            <div className="space-y-2">
              <p>
                • <strong>카테고리별 관리:</strong> 각 섹션의 언어팩을 개별 관리
              </p>
              <p>
                • <strong>일괄 편집:</strong> 여러 언어팩을 한 번에 수정
              </p>
              <p>
                • <strong>자동 번역:</strong> Google Translate API 연동
              </p>
            </div>
            <div className="space-y-2">
              <p>
                • <strong>실시간 반영:</strong> 수정 즉시 사이트에 적용
              </p>
              <p>
                • <strong>버전 관리:</strong> 변경 이력 자동 저장
              </p>
              <p>
                • <strong>검색 기능:</strong> 키워드로 빠른 검색
              </p>
            </div>
          </div>
        </div>
