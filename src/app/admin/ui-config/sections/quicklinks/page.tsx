...
export default function QuickLinksSectionEditPage() {
      ...
      if (response.ok) {
        ...
        if (data.sections?.quicklinks) {
          ...
          if (quicklinksSection.data?.links) {
            const convertedLinks = quicklinksSection.data.links.map(
              (link: any) => ({
                id: link.id,
  ...
  const handleSave = async () => {
    ...
    if (autoTranslate) {
      const needsTranslation = quickLinks.some(
        (link) => !link.titleEn || !link.titleJp,
      );
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  메인 페이지에 표시되는 빠른 접근 링크를 관리합니다 (최대 3개)
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={link.titleEn || ""}
                        onChange={(e) =>
                          handleUpdateLink(link.id, { titleEn: e.target.value })
                        }
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={link.titleJp || ""}
                        onChange={(e) =>
                          handleUpdateLink(link.id, { titleJp: e.target.value })
                        }
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      ...
                      <div className="flex flex-wrap gap-2">
                        {emojiSuggestions.map((emoji) => (
                          <button
                            key={emoji}
                            onClick={() =>
                              handleUpdateLink(link.id, { icon: emoji })
                            }
        ...
        {quickLinks.length < 3 && (
          ...
          >
            <Plus className="w-5 h-5" />새 링크 추가
          </button>
        )}
        ...
        {autoTranslate && (
          <div className="mt-8 bg-blue-50 rounded-xl p-4">
            <p className="text-sm text-blue-800">
              <strong>💡 자동 번역 활성화됨:</strong> 저장 시 입력한 한글 링크
              제목이 자동으로 영어와 일본어로 번역됩니다. 이모지는 그대로
              유지됩니다.
            </p>
          </div>
        )}
