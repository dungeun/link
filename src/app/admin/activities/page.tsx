  ...
  useEffect(() => {
    let filtered = activities.filter((activity) => {
      ...
      const matchesUserType =
        filterUserType === "all" || activity.userType === filterUserType;
      ...
      <div className="space-y-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            플랫폼 내 모든 사용자의 활동을 모니터링하고 추적합니다.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm">
            ...
            <h3 className="text-2xl font-bold text-gray-900">
              {(activityTypeStats.campaign_create || 0) +
                (activityTypeStats.campaign_apply || 0)}
            </h3>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm">
            ...
            <h3 className="text-2xl font-bold text-gray-900">
              {(activityTypeStats.payment || 0) +
                (activityTypeStats.settlement || 0)}
            </h3>
                  ...
                  <tr key={activity.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <span
                          className={`px-2 py-1 text-xs rounded-full mr-2 ${getUserTypeColor(activity.userType)}`}
                        >
                          {activity.userType === "ADMIN"
                            ? "관리자"
                            : activity.userType === "BUSINESS"
                              ? "비즈니스"
                              : "인플루언서"}
                        </span>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <span
                          className={`px-3 py-1 text-sm rounded-full flex items-center ${getActivityColor(activity.type)}`}
                        >
                          <span className="mr-1">
                      ...
                      {activity.metadata && (
                        <div className="text-xs text-gray-500 mt-1">
                          {activity.metadata.campaignTitle && (
                            <span className="mr-3">
                              캠페인: {activity.metadata.campaignTitle}
                            </span>
                          )}
                          {activity.metadata.amount && (
                            <span className="mr-3">
                              금액: ₩{activity.metadata.amount.toLocaleString()}
                            </span>
                          )}
