...
export default function ContentDetailPage({ params }: ContentDetailPageProps) {
  ...
  const handleStatusUpdate = async (newStatus: "APPROVED" | "REJECTED") => {
    try {
      ...
      if (response.ok) {
        ...
        alert(
          `콘텐츠가 ${newStatus === "APPROVED" ? "승인" : "거절"}되었습니다.`,
        );
      } else {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-sm font-medium ${statusStyles[status as keyof typeof statusStyles] || "bg-gray-100 text-gray-800"}`}
      >
              ...
              <div className="space-y-4">
                ...
                {content.media && content.media.length > 0 && (
                  <div>
                    ...
                    {content.media.filter((item) => item.type === "image")
                      .length === 0 && (
