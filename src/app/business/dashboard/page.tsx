          ...
          <div className="container mx-auto px-4 sm:px-6 py-8 sm:py-12">
            <div className="max-w-4xl">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4">
                {t(
                  "business.dashboard.greeting",
                  "안녕하세요, {name}님! 👋",
                ).replace(
                  "{name}",
                  user?.name ||
                    user?.email ||
                    t("business.default_name", "비즈니스"),
                )}
              </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
            <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
              <div className="flex items-start justify-between mb-3 sm:mb-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs sm:text-sm font-medium text-gray-600 truncate">
                    {t("business.stats.total_campaigns", "전체 캠페인")}
                  </h3>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">
                {t("business.stats.total_campaigns_desc", "총 캠페인 수")}
              </p>
                  ...
                  {growthData.campaigns.value > 0 ? (
                    <>
                      ...
                      <span className="truncate">
                        +{growthData.campaigns.value}%{" "}
                        {t("business.stats.vs_last_month", "지난달 대비")}
                      </span>
                    </>
                  ) : growthData.campaigns.value < 0 ? (
                    <>
                      <svg
                        className="w-3 h-3 mr-1 flex-shrink-0 text-red-500"
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                        <path
                    ...
                    <span className="text-gray-500 truncate">
                      {t("business.stats.no_change", "변동 없음")}
                    </span>
                  )}
                </span>
              </div>
            ...
            <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
              <div className="flex items-start justify-between mb-3 sm:mb-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs sm:text-sm font-medium text-gray-600 truncate">
                    {t("business.stats.active_campaigns", "진행중 캠페인")}
                  </h3>
                  <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1 sm:mt-2">
              ...
              <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">
                {t("business.stats.active_campaigns_desc", "현재 진행중")}
              </p>
                ...
                <span className="inline-flex items-center">
                  {growthData.activeCampaigns.value > 0 ? (
                    <>
                      ...
                      <span className="truncate">
                        +{growthData.activeCampaigns.value}%{" "}
                        {t("business.stats.vs_last_week", "지난주 대비")}
                      </span>
                    </>
                  ) : growthData.activeCampaigns.value < 0 ? (
                    <>
            ...
            <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
              <div className="flex items-start justify-between mb-3 sm:mb-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs sm:text-sm font-medium text-gray-600 truncate">
                    {t("business.stats.total_applicants", "총 지원자")}
                  </h3>
                  <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-1 sm:mt-2">
              ...
              <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">
                {t("business.stats.total_applicants_desc", "누적 지원자")}
              </p>
                ...
                <span className="inline-flex items-center">
                  {growthData.applications.value > 0 ? (
                    <>
                      ...
                      <span className="truncate">
                        +{growthData.applications.value}%{" "}
                        {t("business.stats.this_month", "이번달")}
                      </span>
                    </>
                  ) : growthData.applications.value < 0 ? (
                    <>
            ...
            <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5">
              <div className="flex items-start justify-between mb-3 sm:mb-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs sm:text-sm font-medium text-gray-600 truncate">
                    {t("business.stats.total_spent", "총 지출")}
                  </h3>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">
                {t("business.stats.total_spent_desc", "누적 집행 금액")}
              </p>
              <div className="text-xs text-blue-600">
                <span className="inline-flex items-center">
                  <span className="truncate">
                    {growthData.roi > 0
                      ? t(
                          "business.stats.roi_achieved",
                          "ROI {value}% 달성",
                        ).replace("{value}", growthData.roi.toString())
                      : t(
                          "business.stats.no_roi_data",
                          "아직 ROI 데이터가 없습니다",
                        )}
                  </span>
                </span>
