...
export async function GET(
  ...
  try {
    ...
    if (!campaignId || typeof campaignId !== "string") {
      const errorResponse = ApiResponseService.badRequest(
        "Invalid campaign ID",
      );
    ...
    const legacyFormatted = {
      ...
      imageUrl: response.data.images.find((img: any) => img.type === "MAIN")
        ?.url,
      headerImageUrl: response.data.images.find(
        (img: any) => img.type === "HEADER",
      )?.url,
      thumbnailImageUrl: response.data.images.find(
        (img: any) => img.type === "THUMBNAIL",
      )?.url,
      detailImages: response.data.images.filter(
        (img: any) => img.type === "DETAIL",
      ),
      productImages: response.data.images.filter(
      ...
      // 사용자 상호작용 (있는 경우)
      ...(response.data.userInteraction && {
        userInteraction: response.data.userInteraction,
      }),
...
export async function PUT(
  ...
  try {
    ...
    const updatedCampaign = await prisma.campaign.update({
      ...
      data: {
        ...
        ...(body.applicationEndDate && {
          applicationEndDate: new Date(body.applicationEndDate),
        }),
        updatedAt: new Date(),
      },
    });
    ...
    // 정규화된 데이터 업데이트
    if (body.hashtags) {
      await CampaignNormalizedService.updateCampaignHashtags(
        campaignId,
        body.hashtags,
      );
    }
    ...
    if (body.platforms) {
      await CampaignNormalizedService.updateCampaignPlatforms(
        campaignId,
        body.platforms,
      );
    }
    ...
    if (body.images) {
      await CampaignNormalizedService.updateCampaignImages(
        campaignId,
        body.images,
      );
    }
    ...
    if (body.keywords) {
      await CampaignNormalizedService.updateCampaignKeywords(
        campaignId,
        body.keywords,
      );
    }
    ...
    if (body.questions) {
      await CampaignNormalizedService.updateCampaignQuestions(
        campaignId,
        body.questions,
      );
    }
