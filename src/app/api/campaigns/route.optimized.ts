...
import {
  ApiResponseService,
  NextApiResponseHelper,
} from "@/lib/services/api-response.service";
...
const campaignListSchema = z.object({
  ...
  sortBy: z
    .enum(["created", "budget", "endDate", "applications"])
    .default("created"),
    ...
    if (response.success && response.data) {
      const legacyFormatted = {
        campaigns: response.data.map((campaign: any, index: number) => ({
          ...
          imageUrl:
            campaign.images.find((img: any) => img.type === "THUMBNAIL")?.url ||
