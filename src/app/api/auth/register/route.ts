...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (userData.type === "BUSINESS") {
        ...
        const businessProfileData = {
          ...
          businessNumber,
          businessCategory:
            (formData.get("businessCategory") as string) || "기타",
          representativeName:
            (formData.get("representativeName") as string) || userData.name,
          businessAddress:
            (formData.get("businessAddress") as string) || userData.address,
        };

        // 비즈니스 프로필 유효성 검사
        const businessValidation = await ValidationHelper.validate(
          businessProfileData,
          businessProfileSchema,
        );
        ...
        if (businessFile && businessFile instanceof File) {
          ...
          // 파일 업로드
          businessFileUrl = await uploadFile(
            businessFile,
    ...
    // 사용자 등록 데이터 유효성 검사
    const userValidation = await ValidationHelper.validate(
      userData,
      userRegisterSchema,
    );
    if (!userValidation.success) {
      ...
      return NextResponse.json(
        { error: "사용자 정보가 유효하지 않습니다.", details: errors },
        { status: 400 },
      );
    }
