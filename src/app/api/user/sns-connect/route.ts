...
async function fetchFollowerCount(
  platform: string,
  username: string,
): Promise<{ followers: number; todayVisitors?: number }> {
  try {
    let stats = null;
    ...
    if (stats && stats.followers > 0) {
      console.log(
        `[${platform}] Scraping success for ${username}: ${stats.followers} followers`,
      );
...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalFollowers =
      ...
      snsAccounts.reduce((sum, acc) => sum + (acc.followers || 0), 0);
    ...
    if (!disconnect && username) {
      ...
      switch (platform) {
        ...
        case "naverBlog":
          ...
          console.log(`[POST] Naver Blog update data:`, {
            followers,
            todayVisitors,
          });
          break;
      ...
      if (currentProfile) {
        followerUpdateData.followerCount =
          (platform === "instagram"
            ? followers
            : currentProfile.instagramFollowers || 0) +
          (platform === "youtube"
            ? followers
            : currentProfile.youtubeSubscribers || 0) +
          (platform === "tiktok"
            ? followers
            : currentProfile.tiktokFollowers || 0) +
          (platform === "naverBlog"
            ? followers
            : currentProfile.naverBlogFollowers || 0);
...
export async function PUT(req: NextRequest) {
  try {
      ...
      if (username) {
        ...
        switch (plat) {
          ...
          case "naverBlog":
            ...
            console.log(`[PUT] Naver Blog update data:`, {
              followers,
              todayVisitors,
            });
            break;
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      // 업데이트된 값으로 덮어쓰기
      if (updateData.instagramFollowers !== undefined)
        currentFollowers.instagram = Number(updateData.instagramFollowers);
      if (updateData.youtubeSubscribers !== undefined)
        currentFollowers.youtube = Number(updateData.youtubeSubscribers);
      if (updateData.tiktokFollowers !== undefined)
        currentFollowers.tiktok = Number(updateData.tiktokFollowers);
      if (updateData.naverBlogFollowers !== undefined)
        currentFollowers.naverBlog = Number(updateData.naverBlogFollowers);
