...
export async function POST(request: NextRequest) {
  try {
    ...
    if (success) {
      ...
      logger.info(`JSON cache updated successfully: ${type} - paths: ${updatePaths.join(', ')} - dataSize: ${JSON.stringify(data).length}bytes`)
    ...
    logger.error(`JSON cache update failed: ${error instanceof Error ? error.message : String(error)}`)
