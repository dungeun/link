...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const isApiKeyValid =
        await googleTranslateService.validateApiKey(apiKeyToValidate);
      ...
      for (const targetLang of targetLanguages) {
        try {
          ...
          console.log(
            `[API ${testMode ? "Test" : "Normal"}] 번역 수행: ${text} -> ${targetLang}`,
          );
    ...
    if (error instanceof Error) {
      if (error.message.includes("API key not configured")) {
        ...
        errorMessage =
          "Google Translate API에 연결할 수 없습니다. 네트워크를 확인해주세요.";
      } else {
