      ...
      (acc, setting) => {
        switch (setting.key) {
          ...
          case "google_translate_default_target_langs":
            acc.defaultTargetLangs = setting.value
  ...
  try {
    ...
    updates.push(
      prisma.siteConfig.upsert({
        ...
        create: {
          key: "google_translate_default_source_lang",
      ...
      prisma.siteConfig.upsert({
        ...
        create: {
          key: "google_translate_default_target_langs",
          value: JSON.stringify(defaultTargetLangs),
        },
      }),
      prisma.siteConfig.upsert({
        ...
        create: {
          key: "google_translate_auto_translate_on_create",
          value: autoTranslateOnCreate.toString(),
        },
      }),
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    try {
      ...
      // 간단한 번역 테스트
      const testResult = await googleTranslateService.translateText(
        "안녕하세요",
        "en",
        "ko",
      );
      ...
      if (testResult && testResult.text) {
        ...
        return NextResponse.json(
          {
            error:
              "API 키 테스트에 실패했습니다. 번역 결과를 받을 수 없습니다.",
          },
          { status: 400 },
        );
      }
      ...
      if (error instanceof Error) {
        if (error.message.includes("API key not configured")) {
          ...
          errorMessage =
            "Google Translate API에 연결할 수 없습니다. 네트워크 연결을 확인해주세요.";
