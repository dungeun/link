...
// GET /api/admin/analytics - 통계 데이터 조회 (캐싱 최적화)
export async function GET(request: NextRequest) {
  try {
    ...
    const cached = await AdminCache.getAnalytics(cacheKey);
    if (cached) {
    ...
    const previousRevenue = await prisma.payment.aggregate({
      where: {
        ...
        createdAt: {
          lt: startDate,
          gte: new Date(
            startDate.getTime() - (new Date().getTime() - startDate.getTime()),
          ),
        },
      },
      _sum: {
    ...
    const totalSettlements = await prisma.settlement
      .aggregate({
        where: {
          status: "COMPLETED",
          createdAt: {
    ...
    campaignsByCategory.forEach((campaign) => {
      const category =
        campaign.business.businessProfile?.businessCategory || "기타";
    ...
    const formattedInfluencers = topInfluencers
      .map((influencer) => ({
        id: influencer.id,
        name: influencer.name,
        email: influencer.email,
        campaigns: influencer.applications.length,
        earnings: influencer.applications.reduce(
          (sum, app) => sum + (app.campaign?.budget || 0) * 0.8,
          0,
        ),
      }))
    ...
    // 캐시 저장
    await AdminCache.setAnalytics(responseData, cacheKey);
