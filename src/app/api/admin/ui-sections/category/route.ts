...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (autoTranslate && content?.categories) {
      try {
        translations.en = {
          categories: await Promise.all(content.categories.map(async (cat: any) => ({
            ...cat,
            name: cat.name ? await translateText(cat.name, 'ko', 'en').catch(() => cat.name) : cat.name,
            badge: cat.badge ? await translateText(cat.badge, 'ko', 'en').catch(() => cat.badge) : cat.badge
          })))
        };
        ...
        translations.jp = {
          categories: await Promise.all(content.categories.map(async (cat: any) => ({
            ...cat,
            name: cat.name ? await translateText(cat.name, 'ko', 'ja').catch(() => cat.name) : cat.name,
            badge: cat.badge ? await translateText(cat.badge, 'ko', 'ja').catch(() => cat.badge) : cat.badge
          })))
        };
