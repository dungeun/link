...
export async function GET(request: NextRequest) {
  try {
    ...
    // 카테고리별로 언어팩 키 조회
    const category =
      request.nextUrl.searchParams.get("category") || "ui_config";
    ...
    const headerMenuKeys = languagePacks.filter(
      (pack) =>
    ...
    return NextResponse.json({
      ...
      categories: {
        ...
        category: languagePacks.filter((pack) =>
          pack.key.startsWith("category."),
        ),
        quicklink: languagePacks.filter((pack) =>
          pack.key.startsWith("quicklink."),
        ),
