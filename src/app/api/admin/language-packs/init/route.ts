...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      categories: {
        campaigns: sampleLanguagePacks.filter(p => p.category === 'campaigns').length,
        posts: sampleLanguagePacks.filter(p => p.category === 'posts').length,
        menus: sampleLanguagePacks.filter(p => p.category === 'menus').length,
        sections: sampleLanguagePacks.filter(p => p.category === 'sections').length
      }
    });
    ...
    return NextResponse.json(
      { error: '언어팩 초기화에 실패했습니다.', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  } finally {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      translationRate: total > 0 ? Math.round((translationStats._count.id / total) * 100) : 0,
      categories: stats.reduce((acc, stat) => {
        acc[stat.category] = stat._count.id;
        return acc;
      }, {} as Record<string, number>),
      subcategories: subStats.reduce((acc, stat) => {
