          ...
          columns: [
            {
              ...
              links: [
                {
                  id: "link-1",
                  label: "footer.link.find_influencers",
                  href: "/influencers",
                  order: 1,
                  visible: true,
                },
                {
                  id: "link-2",
                  label: "footer.link.create_campaign",
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (config.mainPage?.sectionOrder) {
      ...
      const cleanedSectionOrder = config.mainPage.sectionOrder.filter(
    ...
    if (config.mainPage?.customSections) {
      ...
      const cleanedCustomSections = config.mainPage.customSections.filter(
        (section: Record<string, unknown>) => {
          const sectionId = section.id as string;
