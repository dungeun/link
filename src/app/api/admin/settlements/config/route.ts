    ...
    return NextResponse.json({
      ...
      data: {
        platformFeeRate: platformFeeRate
          ? parseFloat(platformFeeRate.value)
          : 0.2,
        minSettlementAmount: minSettlementAmount
          ? parseInt(minSettlementAmount.value)
          : 10000,
        settlementPeriodDays: settlementPeriod
          ? parseInt(settlementPeriod.value)
          : 7,
        autoSettlementEnabled: autoSettlementEnabled
          ? autoSettlementEnabled.value === "true"
          : false,
      },
    });
  } catch (error) {
