...
export async function GET(request: NextRequest) {
  try {
    ...
    const language = searchParams.get('language') as 'ko' | 'en' | 'jp' || 'ko'
    ...
    logger.error(`Error fetching homepage sections: ${error instanceof Error ? error.message : String(error)}`)
    return NextResponse.json({
  ...
  try {
    ...
    if (action === 'translate') {
      ...
      if (!sectionId || !data || !targetLanguage) {
        return NextResponse.json({
          error: 'sectionId, data, and targetLanguage are required for translation',
          success: false
        }, { status: 400 })
      }
    ...
    logger.error(`Error in homepage section POST: ${error instanceof Error ? error.message : String(error)}`)
    return NextResponse.json({
...
export async function PUT(request: NextRequest) {
  try {
    ...
    logger.error(`Error in homepage section PUT: ${error instanceof Error ? error.message : String(error)}`)
    return NextResponse.json({
      error: 'Failed to process request',
      success: false
    }, { status: 500 })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    logger.error(`Error hiding homepage section: ${error instanceof Error ? error.message : String(error)}`)
    return NextResponse.json({
      error: 'Failed to hide section',
      success: false
    }, { status: 500 })
