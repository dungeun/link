...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tossSecretKey) {
      ...
      return NextResponse.redirect(
        new URL(
          "/business/campaigns/new?error=server_config_error",
          request.url,
        ),
      );
    }
    ...
    if (!confirmResponse.ok) {
      ...
      return NextResponse.redirect(
        new URL(
          "/business/campaigns/new?error=payment_confirm_failed",
          request.url,
        ),
      );
    }
    ...
    return NextResponse.redirect(
      new URL(
        `/business/campaigns/${payment.campaignId}?payment=success`,
        request.url,
      ),
    );
  } catch (error) {
