...
export async function GET(request: NextRequest) {
  try {
        ...
        const defaultFaq = [
          {
            ...
            answer:
              "LinkPick는 브랜드와 인플루언서를 연결하는 AI 기반 마케팅 플랫폼입니다. 정밀한 매칭 알고리즘으로 최적의 파트너를 찾아드립니다.",
            order: 1,
          },
          {
            question: "수수료는 얼마인가요?",
            answer:
              "캠페인 성공 시에만 거래액의 10%를 수수료로 받습니다. 회원가입과 플랫폼 이용은 무료입니다.",
            order: 2,
          },
          {
            question: "어떤 카테고리의 인플루언서가 있나요?",
            answer:
              "패션, 뷰티, 푸드, 여행, 테크, 게임 등 20개 이상의 카테고리에서 다양한 인플루언서가 활동하고 있습니다.",
            order: 3,
          },
          {
            question: "캠페인 진행 과정은 어떻게 되나요?",
            answer:
              "캠페인 등록 → 인플루언서 매칭 → 협의 및 계약 → 콘텐츠 제작 → 성과 측정의 5단계로 진행됩니다.",
            order: 4,
          },
        ];
      ...
      if (testimonialItems.length === 0) {
        const defaultTestimonials = [
          ...
          },
          {
            ...
            rating: 5,
            avatar:
              "https://images.unsplash.com/photo-1494790108755-2616c9c3e0e6?w=60&h=60&fit=crop&crop=face",
            order: 2,
          },
          {
            name: "테크기업 B",
    ...
    const [faqResult, testimonialResult] = await Promise.all([
      fetch(`${request.nextUrl.origin}/api/home/content?type=faq`).then((r) =>
        r.json(),
      ),
      fetch(
        `${request.nextUrl.origin}/api/home/content?type=testimonials`,
      ).then((r) => r.json()),
    ]);
