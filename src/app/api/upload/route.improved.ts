...
async function validateImageDimensions(
  ...
  try {
    ...
    if (metadata.height && metadata.height > MAX_IMAGE_HEIGHT_PX) {
      errors.push(
        `이미지 높이가 ${MAX_IMAGE_HEIGHT_PX}px를 초과합니다. (현재: ${metadata.height}px)`,
      );
    }
    ...
    if (metadata.width && metadata.width > MAX_IMAGE_HEIGHT_PX) {
      errors.push(
        `이미지 너비가 ${MAX_IMAGE_HEIGHT_PX}px를 초과합니다. (현재: ${metadata.width}px)`,
      );
    }
    ...
    if (metadata.format && !allowedFormats.includes(metadata.format)) {
      errors.push(
        `지원하지 않는 이미지 형식입니다. (허용: ${allowedFormats.join(", ")})`,
      );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!file) {
      return createErrorResponse(createApiError.validation("파일이 없습니다."));
    }
    ...
    if (!uploadValidation.success) {
      const errors = ValidationHelper.extractFieldErrors(
        uploadValidation.errors!,
      );
      return createErrorResponse(
        createApiError.validation(
          "파일 업로드 설정이 유효하지 않습니다.",
          errors,
        ),
      );
    }
    ...
    if (type === "profile" || type === "campaign" || type === "content") {
      // 이미지 파일 기본 검증
      const imageValidation = ValidationHelper.validateImageFile(
        file,
        MAX_FILE_SIZE_MB,
      );
      if (!imageValidation.valid) {
        return createErrorResponse(
          createApiError.validation("이미지 파일 검증 실패", {
      ...
      if (!dimensionValidation.valid) {
        return createErrorResponse(
          createApiError.validation("이미지 크기 제한 초과", {
    ...
    // 문서 파일인 경우 특별 검증
    if (type === "document") {
      const docValidation = ValidationHelper.validateDocumentFile(
        file,
        MAX_FILE_SIZE_MB,
      );
      if (!docValidation.valid) {
        return createErrorResponse(
          createApiError.validation("문서 파일 검증 실패", {
