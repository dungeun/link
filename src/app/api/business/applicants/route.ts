...
export async function GET(req: NextRequest) {
  try {
    ...
    // 모든 캠페인의 지원자 조회
    console.log(
      "Searching for applications with campaign IDs:",
    ...
    applications.forEach((app) => {
      console.log(
        `  - ${app.influencer.name} applied to ${app.campaign.title}`,
      );
    });
    ...
    const applicants = applications.map((app) => ({
      ...
      influencerHandle:
        app.influencer.profile?.instagram ||
        app.influencer.profile?.youtube ||
        "",
      ...
      influencer: {
        ...
        profile: app.influencer.profile
          ? {
              ...
              naverBlogTodayVisitors:
                app.influencer.profile.naverBlogTodayVisitors,
              averageEngagementRate:
                app.influencer.profile.averageEngagementRate,
