...
export async function GET(request: NextRequest) {
  try {
    ...
    // 지난달 캠페인 수
    const lastMonthCampaigns = await prisma.campaign
      .count({
        where: {
          businessId: userId,
          createdAt: {
    ...
    // 이번달 캠페인 수
    const thisMonthCampaigns = await prisma.campaign
      .count({
        where: {
          businessId: userId,
          createdAt: {
    ...
    // 지난주 활성 캠페인 수
    const lastWeekActiveCampaigns = await prisma.campaign
      .count({
        where: {
          businessId: userId,
          status: "ACTIVE",
    ...
    // 이번주 활성 캠페인 수
    const thisWeekActiveCampaigns = await prisma.campaign
      .count({
        where: {
          businessId: userId,
          status: "ACTIVE",
    ...
    // 이번달 지원자 수
    const thisMonthApplications = await prisma.campaignApplication
      .count({
        where: {
          campaign: {
            businessId: userId,
    ...
    // 지난달 지원자 수
    const lastMonthApplications = await prisma.campaignApplication
      .count({
        where: {
          campaign: {
            businessId: userId,
    ...
    const totalSpent = await prisma.payment
      .aggregate({
        where: {
          userId: userId,
          status: "COMPLETED",
    ...
    const totalRevenue = await prisma.campaign
      .aggregate({
        where: {
          businessId: userId,
          status: "COMPLETED",
    ...
    const campaignGrowth =
      lastMonthCampaigns > 0
        ? Math.round(
            ((thisMonthCampaigns - lastMonthCampaigns) / lastMonthCampaigns) *
              100,
          )
        : thisMonthCampaigns > 0
          ? 100
          : 0;

    const activeCampaignGrowth =
      lastWeekActiveCampaigns > 0
        ? Math.round(
            ((thisWeekActiveCampaigns - lastWeekActiveCampaigns) /
              lastWeekActiveCampaigns) *
              100,
          )
        : thisWeekActiveCampaigns > 0
          ? 100
          : 0;
