  ...
  try {
    ...
    if (campaign.hashtags) {
      try {
        ...
        hashtags = (campaign.hashtags as string)
          .split(" ")
          .filter((tag) => tag);
      }
    }
    ...
    try {
      detailImages = campaign.detailImages
        ? JSON.parse(String(campaign.detailImages))
        : [];
    } catch (e) {
      detailImages = [];
    }
...
export async function PUT(
  ...
  try {
    ...
    if (
      existingCampaign.businessId !== (user.userId || user.id) &&
      userType !== "admin"
    ) {
      return NextResponse.json(
    ...
    if (body.targetFollowers !== undefined)
      updateData.targetFollowers = body.targetFollowers;
    ...
    if (body.detailImages !== undefined)
      updateData.detailImages = JSON.stringify(body.detailImages);
