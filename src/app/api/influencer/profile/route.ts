...
export async function GET(request: NextRequest) {
  try {
    ...
    const checkProfileCompleted = (profileData: any) => {
      ...
      const isRealNameValid =
        profileData.realName && String(profileData.realName).trim() !== "";
      const isBirthYearValid =
        profileData.birthYear &&
        Number(profileData.birthYear) > 1900 &&
        Number(profileData.birthYear) <= new Date().getFullYear();
      const isGenderValid =
        profileData.gender &&
        ["male", "female", "other", "남성", "여성", "기타"].includes(
      ...
      const isCompleted =
        isRealNameValid && isBirthYearValid && isGenderValid && isPhoneValid;

      console.log("Profile completion check (GET):", {
        realName: {
          valid: isRealNameValid,
          value: profileData.realName,
          type: typeof profileData.realName,
        },
        birthYear: {
          valid: isBirthYearValid,
          value: profileData.birthYear,
          type: typeof profileData.birthYear,
        },
        gender: {
          valid: isGenderValid,
          value: profileData.gender,
          type: typeof profileData.gender,
        },
        phone: {
          valid: isPhoneValid,
          value: profileData.phone,
          type: typeof profileData.phone,
        },
        result: isCompleted,
      });
  ...
  try {
      ...
      if (contentType?.includes("multipart/form-data")) {
        ...
        if (profileString) {
          ...
          data = {
            ...
            addressData: formData.get("addressData")
              ? JSON.parse(formData.get("addressData") as string)
              : null,
    ...
    if (cleanedData.bankingInfo) {
      if (cleanedData.bankingInfo.accountType === "domestic") {
        extractedBankName =
          cleanedData.bankingInfo.domestic?.bankName || cleanedData.bankName;
        extractedBankAccountNumber =
          cleanedData.bankingInfo.domestic?.accountNumber ||
          cleanedData.bankAccountNumber;
        extractedBankAccountHolder =
          cleanedData.bankingInfo.domestic?.accountHolder ||
          cleanedData.bankAccountHolder;
      } else if (cleanedData.bankingInfo.accountType === "international") {
        extractedBankName =
          cleanedData.bankingInfo.international?.bankEnglishName ||
          cleanedData.bankName;
        extractedBankAccountNumber =
          cleanedData.bankingInfo.international?.accountNumber ||
          cleanedData.bankAccountNumber;
        extractedBankAccountHolder =
          cleanedData.bankingInfo.international?.englishName ||
          cleanedData.bankAccountHolder;
      }
    }
    ...
    const checkProfileCompleted = (profileData: {
      ...
      // 필수 필드들 검증 (더 유연한 조건)
      const isRealNameValid =
        currentRealName && String(currentRealName).trim() !== "";
      const isBirthYearValid =
        currentBirthYear &&
        Number(currentBirthYear) > 1900 &&
        Number(currentBirthYear) <= new Date().getFullYear();
      const isGenderValid =
        currentGender &&
        ["male", "female", "other", "남성", "여성", "기타"].includes(
      ...
      const isCompleted =
        isRealNameValid && isBirthYearValid && isGenderValid && isPhoneValid; // && isAddressValid;

      console.log("Profile completion check:", {
        realName: {
          valid: isRealNameValid,
          value: currentRealName,
          type: typeof currentRealName,
        },
        birthYear: {
          valid: isBirthYearValid,
          value: currentBirthYear,
          type: typeof currentBirthYear,
        },
        gender: {
          valid: isGenderValid,
          value: currentGender,
          type: typeof currentGender,
        },
        phone: {
          valid: isPhoneValid,
          value: currentPhone,
          type: typeof currentPhone,
        },
        address: {
          valid: isAddressValid,
          value: currentAddress,
          type: typeof currentAddress,
        },
        result: isCompleted,
      });
