  ...
  const loadInfluencers = async () => {
    try {
      ...
      if (selectedCategory !== "all")
        searchParams.set("categories", selectedCategory);
            ...
            {loading ? (
              ...
              influencers.map((influencer, index) => {
                ...
                return (
                  <div
                    key={influencer.id}
                    className="influencer-card bg-white rounded-xl shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 animate-in fade-in slide-in-from-bottom-4 duration-600"
                        ...
                        <div className="flex items-center">
                          <img
                            src={
                              ...
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(influencer.name)}&background=6366f1&color=fff`
                            }
                            alt={influencer.name}
                          ...
                          <div>
                            ...
                            <p className="text-gray-500 text-sm">
                              {influencer.profile?.instagram ||
                                `@${influencer.name.toLowerCase().replace(/\s+/g, "_")}`}
                            </p>
                          </div>
                        </div>
                        <span className="text-sm font-medium text-cyan-600 bg-cyan-50 px-3 py-1 rounded-full">
                          {categories.find((c) => c.id === mainCategory)
                            ?.name || "라이프스타일"}
                        </span>
                      </div>
