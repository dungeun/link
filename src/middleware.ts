...
export async function middleware(request: NextRequest) {
  ...
  if (process.env.LOG_LEVEL === "debug") {
    logger.debug("Middleware: Request received", {
      path: pathname,
  ...
  if (publicPaths.some((path) => pathname.startsWith(path))) {
    return pathname.startsWith("/api/")
      ? await addApiSecurityHeaders(response, request)
      : await addSecurityHeaders(response, request);
  }
  ...
  if (protectedPagePaths.some((path) => pathname.startsWith(path))) {
    ...
    try {
      ...
      if (perfLogger)
        perfLogger.end({ status: 302, redirect: "/login", error: true });
      return NextResponse.redirect(new URL("/login", request.url));
    }
  }
