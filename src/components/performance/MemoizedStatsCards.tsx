  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`p-3 ${colorClasses[color as keyof typeof colorClasses]} rounded-full`}
        >
          {icon}
        </div>
      </div>
...
const MemoizedStatsCards = memo(function MemoizedStatsCards({
  stats,
}: MemoizedStatsCardsProps) {
  // 아이콘들을 메모이제이션
  const icons = useMemo(
    () => ({
      users: (
