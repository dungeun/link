...
const OptimizedCampaignCard = memo(function OptimizedCampaignCard({
  ...
  const platformIcons = useMemo(() => {
    ...
    return campaign.platforms?.slice(0, 2).map(platform => 
      iconMap[platform] || '📱'
    ) || []
  }, [campaign.platforms])
  ...
  const progressData = useMemo(() => {
    const percentage = Math.min((campaign.applicants / campaign.maxApplicants) * 100, 100)
  ...
  const optimizedImageUrl = useMemo(() => {
    if (!campaign.imageUrl || campaign.imageUrl === '/images/campaigns/default.jpg') {
      // 가상 이미지 URL 생성
      const imageIndex = campaign.id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % 6
      const virtualImages = [
      ...
      <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100 hover:border-gray-200">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-600">{campaign.brand}</span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-medium ${progressData.isAlmostFull ? 'text-orange-600' : 'text-gray-900'}`}>
            ...
            {progressData.remainingSlots <= 5 && progressData.remainingSlots > 0 && (
              <p className="text-xs text-orange-600 font-medium">
                {t?.('campaign.remaining_slots', '남은 자리 {count}개').replace('{count}', progressData.remainingSlots.toString()) || 
