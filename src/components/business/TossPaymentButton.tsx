...
export default function TossPaymentButton({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 클라이언트 키 (환경변수에서 가져오기)
      const clientKey = process.env.NEXT_PUBLIC_TOSS_CLIENT_KEY || 'test_ck_DnyRpQWGrNzO1Ek5W9eVKwv1M9EN'
      
      // TossPayments 인스턴스 생성
      const tossPaymentsInstance = tossPayments(clientKey)

      // 결제창 호출
      await tossPaymentsInstance.requestPayment(paymentMethod === 'card' ? '카드' : '휴대폰', {
        amount,
        orderId,
        orderName,
        customerName,
