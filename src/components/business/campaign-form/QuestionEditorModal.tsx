...
export default function QuestionEditorModal({
  ...
  const [editingQuestions, setEditingQuestions] =
    useState<DynamicQuestion[]>(questions);
  const [selectedQuestionId, setSelectedQuestionId] = useState<string | null>(
    null,
  );

  const selectedQuestion = editingQuestions.find(
  ...
  const updateQuestion = (id: string, updates: Partial<DynamicQuestion>) => {
    setEditingQuestions(
      editingQuestions.map((q) => (q.id === id ? { ...q, ...updates } : q)),
    );
  };
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {editingQuestions
                    .filter((q) => isDefaultQuestion(q.id))
                    .map((question) => (
                      <div
                        key={question.id}
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            <p
                              className={`font-medium text-sm truncate ${question.enabled === false ? "text-gray-400" : ""}`}
                            >
                              {question.question}
                            </p>
