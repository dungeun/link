...
export default function SNSConnection({
  onFollowersUpdate,
}: SNSConnectionProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.followers === 0) {
          alert(
            "계정은 연동되었지만 팔로워 수를 가져올 수 없습니다. 계정명을 다시 확인해주세요.",
          );
        }
        ...
        if (data.followers > 0) {
          alert(
            `SNS 계정이 연동되었습니다! (팔로워: ${data.followers.toLocaleString()}명)`,
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.updated && data.updated.length > 0) {
            ...
            data.updated.forEach(
              (update: {
                ...
                const index = updated.findIndex(
                  (acc) => acc.platform === update.platform,
                );
                if (index !== -1) {
                  updated[index] = {
                    ...updated[index],
          ...
          // 총 팔로워 수 재계산
          const newTotal = snsAccounts.reduce((sum, acc) => {
            const update = data.updated.find(
              (u: {
                platform: string;
                followers: number;
                todayVisitors?: number;
              }) => u.platform === acc.platform,
            );
        ...
        if (data.updated && data.updated.length > 0) {
          const successCount = data.updated.filter(
            (u: { followers: number }) => u.followers > 0,
          ).length;
          if (successCount > 0) {
            alert(`${successCount}개 계정의 팔로워 수가 업데이트되었습니다!`);
          } else {
            alert(
              "팔로워 수를 가져올 수 없습니다. 계정명을 확인하거나 나중에 다시 시도해주세요.",
            );
          }
        } else {
    ...
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="space-y-3">
        {snsAccounts.map((account) => {
          const info =
            platformInfo[account.platform as keyof typeof platformInfo];
          if (!info) return null;
            ...
            <div key={account.platform} className="border rounded-lg p-4">
                ...
                <div className="flex items-center gap-3">
                  {account.isConnected && (
                    <>
                          ...
                          account.platform === "naverBlog" ? (
                            <>
                              ...
                              <p className="text-sm text-gray-700">
                                {(account.todayVisitors || 0).toLocaleString()}
                                <span className="text-xs text-gray-500">
                                  (방문)
                                </span>
                              </p>
                      ...
                      >
                        <RefreshCw
                          className={`w-4 h-4 ${refreshing === account.platform ? "animate-spin" : ""}`}
                        />
                      </button>
                    </>
                  )}
              ...
              {account.isConnected && account.lastUpdated && (
                <p className="text-xs text-gray-400 mt-2">
                  마지막 업데이트:{" "}
                  {new Date(account.lastUpdated).toLocaleString("ko-KR")}
                </p>
              )}
            </div>
          );
        ...
        >
          ...
          >
            <h3 className="text-lg font-bold mb-4">
              {
                platformInfo[selectedPlatform as keyof typeof platformInfo]
                  ?.name
              }{" "}
              계정 연동
            </h3>
