...
function InfluencerMyPage({
  user,
  activeTab,
  setActiveTab,
}: InfluencerMyPageProps) {
  // 언어 훅 사용
  const { t } = useLanguage();
  ...
  const {
    data: savedCampaignsData,
    isLoading: loadingSavedCampaigns,
    refetch: refetchSavedCampaigns,
  } = useSavedCampaigns();
  const { data: applicationsData, isLoading: loadingApplications } =
    useInfluencerApplications();
  ...
  // 출금 관련 상태 - 캐싱된 데이터 사용
  const withdrawals: Withdrawals = withdrawalsData || {
  ...
  const [profileForm, setProfileForm] = useState({
    ...
    categories: profileData?.profile?.categories
      ? parseCategories(profileData.profile.categories)
      : [],
  });
  const [addressData, setAddressData] = useState<AddressData | null>(
    profileData?.profile?.addressData
      ? (profileData.profile.addressData as unknown as AddressData)
      : null,
  );
  ...
  const [campaignActiveTab, setCampaignActiveTab] = useState<
    "all" | "pending" | "active" | "completed" | "reviewing" | "rejected"
  >("all");
  ...
  useEffect(() => {
    if (applications) {
      ...
      const campaignsFromApplications: Campaign[] = approvedApplications.map(
        (app: Application) => ({
  ...
  const getCampaignStatus = (campaign: Campaign) => {
    ...
    if (campaign.status === "in_progress" || campaign.status === "approved")
      return "in_progress";
    return "pending";
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          error.message ||
            t("mypage.profile.saveFailed", "프로필 저장에 실패했습니다."),
        );
      }
    } catch (error) {
  ...
  const handleWithdrawalSubmit = async () => {
    ...
    if (parseInt(withdrawalForm.amount) > withdrawals.withdrawableAmount) {
      alert(
        t(
          "mypage.withdrawal.insufficientFunds",
          "출금 가능 금액을 초과했습니다.",
        ),
      );
      return;
    ...
    try {
      const response = await fetch("/api/influencer/withdrawals", {
        ...
        body: JSON.stringify({
          ...
          bankInfo:
            bankInfo.accountType === "domestic"
              ? bankInfo.domestic
              : bankInfo.international,
        }),
      });
      ...
      if (response.ok) {
        ...
        alert(
          error.message ||
            t("mypage.withdrawal.failed", "출금 신청에 실패했습니다."),
        );
      }
    } catch (error) {
    ...
    switch (activeTab) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-6 rounded-lg shadow">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm text-gray-600">
                      {t("mypage.stats.activeCampaigns", "진행중 캠페인")}
                    </p>
                    <p className="text-2xl font-semibold">
            ...
            {activeCampaigns.length > 0 && (
              <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-lg font-semibold mb-4">
                  {t("mypage.activeCampaigns.title", "진행중인 캠페인")}
                </h3>
            ...
            {recentEarnings.length > 0 && (
              <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-lg font-semibold mb-4">
                  {t("mypage.recentEarnings.title", "최근 수익")}
                </h3>
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="p-6 border-b">
              <h2 className="text-xl font-semibold">
                {t("mypage.campaigns.title", "내 캠페인")}
              </h2>
              <div className="mt-4 flex space-x-4">
                ...
                >
                  {t("mypage.campaign.all", "전체")} (
                  {applications.length + myCampaigns.length})
                </button>
                <button
                ...
                >
                  {t("mypage.campaign.reviewing", "심사중")} (
                  {
                    applications.filter(
                ...
                >
                  {t("mypage.campaign.active", "진행중")} (
                  {
                    myCampaigns.filter((c: Campaign) =>
                      ["approved", "in_progress", "submitted"].includes(
                        c.status,
                      ),
                    ).length
                  }
                ...
                >
                  {t("mypage.campaign.rejected", "거절됨")} (
                  {
                    applications.filter(
                ...
                >
                  {t("mypage.campaign.completed", "완료")} (
                  {
                    myCampaigns.filter(
            ...
            <div className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {t("mypage.campaign.appliedAt", "지원일")}:{" "}
                      ...
                      <div key={app.id} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {t("mypage.campaign.rejectedAt", "거절일")}:{" "}
                              {app.rejectedAt
                ...
                <div className="space-y-4">
                  {myCampaigns
                    .filter((campaign: Campaign) => {
                      ...
                      if (campaignActiveTab === "active")
                        return [
                          "approved",
                          "in_progress",
                          "submitted",
                        ].includes(campaign.status);
                      if (campaignActiveTab === "completed")
              ...
              {((campaignActiveTab === "all" &&
                ...
                (campaignActiveTab === "active" &&
                  myCampaigns.filter((c: Campaign) =>
                    ["approved", "in_progress", "submitted"].includes(c.status),
                  ).length === 0) ||
                (campaignActiveTab === "completed" &&
                  myCampaigns.filter((c: Campaign) => c.status === "completed")
                    .length === 0)) && (
                <div className="text-center py-8">
                  <p className="text-gray-500">
                    {campaignActiveTab === "all" &&
                      t("mypage.campaign.noCampaigns", "캠페인이 없습니다.")}
                    {campaignActiveTab === "reviewing" &&
                      t(
                        "mypage.campaign.noReviewing",
                        "심사중인 캠페인이 없습니다.",
                      )}
                    {campaignActiveTab === "active" &&
                      t(
                        "mypage.campaign.noActive",
                        "진행중인 캠페인이 없습니다.",
                      )}
                    {campaignActiveTab === "rejected" &&
                      t(
                        "mypage.campaign.noRejected",
                        "거절된 캠페인이 없습니다.",
                      )}
                    {campaignActiveTab === "completed" &&
                      t(
                        "mypage.campaign.noCompleted",
                        "완료된 캠페인이 없습니다.",
                      )}
                  </p>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <h2 className="text-xl font-semibold mb-4">
                {t("mypage.earnings.summary", "수익 요약")}
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 border rounded-lg">
                  <p className="text-sm text-gray-600">
                    {t("mypage.earnings.withdrawable", "출금 가능 금액")}
                  </p>
                ...
                <div className="p-4 border rounded-lg">
                  <p className="text-sm text-gray-600">
                    {t("mypage.earnings.pending", "정산 대기중")}
                  </p>
                  <p className="text-2xl font-bold text-yellow-600">
                    ...
                    {withdrawals.settlements
                      ...
                      .reduce(
                        (sum: number, s: Settlement) => sum + s.totalAmount,
                        0,
                      )
                      .toLocaleString()}
                  </p>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-4">
                {t("mypage.withdrawal.title", "출금 신청")}
              </h3>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {t("mypage.withdrawal.maxAmount", "최대 출금 가능")}: ₩
                    {withdrawals.withdrawableAmount.toLocaleString()}
                  </p>
                </div>
                ...
                <button
                  ...
                  disabled={
                    ...
                    parseInt(withdrawalForm.amount) <= 0
                  }
                  className="w-full py-2 px-4 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-400"
                >
                  {submittingWithdrawal
                    ? t("mypage.withdrawal.submitting", "처리중...")
                    : t("mypage.withdrawal.submit", "출금 신청")}
                </button>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-4">
                {t("mypage.settlements.title", "정산 내역")}
              </h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      {t("mypage.profile.selectGender", "선택")}
                    </option>
                    <option value="male">
                      {t("mypage.profile.male", "남성")}
                    </option>
                    <option value="female">
                      {t("mypage.profile.female", "여성")}
                    </option>
                    <option value="other">
                      {t("mypage.profile.other", "기타")}
                    </option>
                  </select>
                </div>
      ...
      case "favorites":
        return (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-semibold mb-6">
              {t("mypage.favorites.title", "관심 캠페인")}
            </h2>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="border-b border-gray-200">
            <nav
              className="-mb-px flex overflow-x-auto space-x-4 sm:space-x-8 px-4 sm:px-6"
              aria-label="Tabs"
            >
              <button
