...
const ActiveCampaignsSection = dynamic(
  () => import("@/components/home/ActiveCampaignsSection"),
  {
    loading: () => (
      <div className="h-64 bg-gray-100 rounded-xl animate-pulse" />
    ),
...
import { UISection, LanguageCode, HeroSlide, QuickLink } from "@/types/global";
...
function OptimizedHomePage({
  ...
  const bannerSlides = useMemo(() => {
    if (
      ...
      Array.isArray(heroSection.content.slides)
    ) {
      return heroSection.content.slides.filter(
        (slide) =>
          slide &&
          typeof slide === "object" &&
          (slide as any).visible !== false,
      );
    }
  ...
  const menuCategories = useMemo(() => {
    if (
      ...
      Array.isArray(categorySection.content.categories)
    ) {
      return categorySection.content.categories.filter(
        (cat) =>
          cat && typeof cat === "object" && (cat as any).visible !== false,
      );
    }
  ...
  // 언어 전환 시간 측정
  const measureLanguageSwitch = useCallback(
    (lang: LanguageCode) => {
        ...
        <div className="bg-gradient-to-r from-green-500 to-blue-500 text-white px-4 py-2 text-sm">
          <div className="container mx-auto flex justify-between items-center">
            ...
            <div className="flex gap-4">
              ...
              <span>
                캐시: {performanceMetrics.cacheHit ? "✅ HIT" : "❌ MISS"}
              </span>
              <span>
                언어 전환: {performanceMetrics.languageSwitch.toFixed(2)}ms
              </span>
