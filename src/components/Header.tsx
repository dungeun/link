...
function Header({ variant = "default" }: HeaderProps) {
  ...
  const isInfluencer = useMemo(
    () => !user || userType === "INFLUENCER" || userType === "USER",
    [user, userType],
  );
  ...
  useEffect(() => {
    ...
    if (savedProfile) {
      try {
        ...
        logger.error("Failed to parse profile", e as Error, {
          module: "Header",
        });
      }
    }
    ...
    const loadHeaderMenus = async () => {
      try {
        ...
        if (response.ok) {
          ...
          logger.error(
            "Failed to load header JSON",
            new Error(`HTTP ${response.status}`),
            { module: "Header" },
          );
        }
      } catch (error) {
        logger.error("Failed to load header menus", error as Error, {
          module: "Header",
        });
      } finally {
  ...
  const getLocalizedText = useCallback(
    (textObj: any, fallback?: string): string => {
      if (!textObj) {
        return fallback || "";
      }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 sm:gap-8 flex-1">
            ...
            <Link href="/" className="flex items-center min-w-0 flex-shrink-0">
              {siteSettings.website.logo &&
              siteSettings.website.logo !== "/logo.svg" ? (
            ...
            <div className="hidden lg:flex items-center gap-4">
              ...
              {isAdmin && (
                <>
                  ...
                  <Link
                    href="/admin/users"
                    className="hover:opacity-80 transition font-medium text-white text-sm"
                  >
            ...
            <div className="flex items-center gap-2">
              {isAuthenticated && user ? (
                ...
                <>
                  <Link
                    href="/login"
                    className="hover:opacity-80 transition text-sm text-white"
                  >
                    {t("menu.login", "로그인")}
                  </Link>
                  <Link
                    href="/register"
                    className="bg-white/20 backdrop-blur px-3 sm:px-4 py-1.5 sm:py-2 rounded-full hover:bg-white/30 transition text-sm text-white"
                  >
                    {t("menu.signup", "회원가입")}
                  </Link>
                </>
