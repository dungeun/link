                ...
                if (images.length === 0) {
                  return (
                    <div className="text-center py-16 bg-gray-50 rounded-xl">
                      ...
                      <h3 className="text-lg font-medium text-gray-900 mb-2">제품 이미지가 없습니다</h3>
                      <p className="text-gray-500">업체에서 제품 이미지를 준비 중입니다.</p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="grid gap-6">
                    {images.map((imageUrl: string, index: number) => (
                      <div key={index} className="relative w-full rounded-xl overflow-hidden bg-gray-50 border border-gray-200">
                        <img
                          src={imageUrl}
                          alt={`제품 이미지 ${index + 1}`}
                          ...
                          onError={(e) => {
                            ...
                            if (parent && !parent.querySelector('.error-message')) {
                              ...
                              errorDiv.className = 'error-message flex flex-col items-center justify-center h-48 text-gray-400 bg-gray-50';
                              errorDiv.innerHTML = `
          ...
          <div className="space-y-8">
            ...
            {campaign.content?.requirements && campaign.content.requirements.trim() && (
              <div className="border border-gray-200 rounded-xl p-6 bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 max-w-4xl">📋 요구사항</h3>
            ...
            {campaign.content?.provisionDetails && campaign.content.provisionDetails.trim() && (
              <div className="border border-gray-200 rounded-xl p-6 bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 max-w-4xl">🎁 제공 내역</h3>
            ...
            {campaign.content?.mission && campaign.content.mission.trim() && (
              <div className="border border-gray-200 rounded-xl p-6 bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 max-w-4xl">🎯 캠페인 미션</h3>
            ...
            {campaign.content?.keywords && Array.isArray(campaign.content.keywords) && campaign.content.keywords.length > 0 && (
              <div className="border border-gray-200 rounded-xl p-6 bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 max-w-4xl">🔑 키워드</h3>
                <div className="flex flex-wrap gap-3 max-w-4xl">
                  {campaign.content.keywords.map((keyword: string, index: number) => (
                    <span key={index} className="px-4 py-2 bg-blue-100 text-blue-700 rounded-lg text-sm font-medium border border-blue-200">
                      {keyword}
                    </span>
                  ))}
                </div>
            ...
            {campaign.content?.hashtags && Array.isArray(campaign.content.hashtags) && campaign.content.hashtags.length > 0 && (
              <div className="border border-gray-200 rounded-xl p-6 bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 max-w-4xl"># 해시태그</h3>
                <div className="flex flex-wrap gap-3 max-w-4xl">
                  {campaign.content.hashtags.map((tag: string, index: number) => (
                    <span key={index} className="px-4 py-2 bg-indigo-100 text-indigo-700 rounded-lg text-sm font-medium border border-indigo-200">
                      #{tag}
                    </span>
                  ))}
                </div>
            ...
            {campaign.content?.additionalNotes && campaign.content.additionalNotes.trim() && (
              <div className="border border-gray-200 rounded-xl p-6 bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 max-w-4xl">📌 추가 안내사항</h3>
