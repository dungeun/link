      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="font-semibold">
              {campaign._count?.applications || 0}/{campaign.target?.maxApplicants || campaign.maxApplicants || 0}명
            </span>
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-semibold">{(campaign.target?.followers?.min || campaign.targetFollowers || 0).toLocaleString()}명</span>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-semibold text-sm">
            {(() => {
              const startDate = campaign.schedule?.campaign?.startDate || campaign.startDate
              const endDate = campaign.schedule?.campaign?.endDate || campaign.endDate
        ...
        {(() => {
          ...
          const startDate = campaign.schedule?.application?.startDate || campaign.applicationStartDate
          const endDate = campaign.schedule?.application?.endDate || campaign.applicationEndDate
        ...
        {/* 인플루언서 발표 */}
        {(() => {
          const announcementDate = campaign.schedule?.announcement?.date || campaign.announcementDate
        ...
        {/* 콘텐츠 등록 기간 */}
        {(() => {
          const contentStartDate = campaign.schedule?.content?.startDate || campaign.contentStartDate
          const contentEndDate = campaign.schedule?.content?.endDate || campaign.contentEndDate
          return (contentStartDate || contentEndDate) && (
            <div className="flex items-center justify-between">
              <span className="text-gray-600 flex items-center gap-2">
        ...
        {/* 결과 발표 */}
        {(() => {
          const resultDate = campaign.schedule?.announcement?.resultDate || campaign.resultAnnouncementDate
          return resultDate && (
            <div className="flex items-center justify-between">
              <span className="text-gray-600 flex items-center gap-2">
            ...
            {campaign.hasApplied === true ? (
              ...
              <Button 
                ...
                disabled={
                  ...
                  !isApplicationPeriodActive || 
                  (campaign._count?.applications || 0) >= (campaign.target?.maxApplicants || campaign.maxApplicants || 0)
                }
              >
                {campaign.status !== 'ACTIVE' 
                  ? '캠페인 종료됨' 
