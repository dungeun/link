...
export function SortableMenuItemWithLanguagePack({
  menu,
  onUpdate,
  onDelete,
    ...
    const fetchLanguagePackKeys = async () => {
      ...
      try {
        const response = await fetch(
          "/api/admin/language-pack-keys?category=ui_config",
          {
            headers: {
        ...
        if (response.ok) {
          ...
          // 현재 선택된 키 찾기
          const currentKey = data.categories.header.find(
            (k: LanguagePackKey) => k.key === menu.label,
          );
          if (currentKey) {
  ...
  const filteredKeys = languagePackKeys.filter(
    (key) =>
