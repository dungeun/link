...
export function SortableMenuItemImproved({
  menu,
  onUpdate,
  onDelete,
}: SortableMenuItemProps) {
  const [languageData, setLanguageData] = useState<LanguagePackData | null>(
    null,
  );
    ...
    const fetchLanguageData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/admin/language-packs/${encodeURIComponent(menu.label)}`,
          {
            headers: {
  ...
  const handleEditName = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          const createResponse = await fetch(
            "/api/admin/language-packs/simple-create",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
      ...
      alert(
        `메뉴 이름 업데이트 중 오류가 발생했습니다: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
      );
    }
  };
      ...
      <div className="grid grid-cols-12 gap-2 items-center p-4">
        ...
        <div className="col-span-3">
          <input
            ...
            placeholder="링크 URL"
            title={
              menu.label.startsWith("category.")
                ? "카테고리 링크는 자동 생성됩니다"
                : "링크 URL을 입력하세요"
            }
