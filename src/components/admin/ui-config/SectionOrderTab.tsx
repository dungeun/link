...
export function SectionOrderTab() {
  const { config, updateSectionOrder, updateMainPageCustomSections } =
    useUIConfigStore();
  ...
  useEffect(() => {
    ...
    const uniqueCustomSections = customSectionsRaw.filter(
      (cs, index, self) => index === self.findIndex((s) => s.id === cs.id),
    );
    ...
    const customSections = uniqueCustomSections.map((cs, index) => {
      let finalId =
        cs.id ||
        `custom-${index}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

      // ID가 이미 사용 중이면 새로운 ID 생성
      if (usedIds.has(finalId)) {
        finalId = `${finalId}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        console.warn(
          `Duplicate custom section ID detected: ${cs.id}, using new ID: ${finalId}`,
        );
      }
    ...
    customSections.forEach((section) => {
      if (!sectionMap.has(section.id)) {
        ...
        console.warn(
          `Duplicate section ID found: ${section.id}, changing to ${newId}`,
        );
        sectionMap.set(newId, { ...section, id: newId });
      }
    });

    const allSections = Array.from(sectionMap.values()).sort(
      (a, b) => a.order - b.order,
    );
