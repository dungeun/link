...
function AdminLayoutContent({ children }: AdminLayoutProps) {
  ...
  useEffect(() => {
    if (!authLoading) {
      ...
      if (
        !isAuthenticated ||
        (user?.type !== "ADMIN" && (user?.type as any) !== "admin")
      ) {
        console.log(
          "AdminLayout - 관리자 권한이 없습니다. 로그인 페이지로 이동합니다.",
        );
        console.log("현재 사용자:", user);

        router.push(
          "/login?error=admin_required&message=관리자 권한이 필요합니다",
        );
      }
      setIsLoading(false);
    }
