...
export function SortableFooterColumn({
  ...
  const handleLinkDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = column.links.findIndex(
        (link) => link.id === String(active.id),
      );
      const newIndex = column.links.findIndex(
        (link) => link.id === String(over.id),
      );

      const newLinks = arrayMove(column.links, oldIndex, newIndex).map(
        (link, index) => ({
          ...link,
          order: index + 1,
        }),
  ...
  const handleLinkUpdate = (
    linkId: string,
    updates: Partial<(typeof column.links)[0]>,
  ) => {
