...
const OptimizedMyPage = memo(({
  ...
  const getStatusBadge = useCallback((status: string) => {
    switch (status) {
      ...
      case 'REJECTED':
        return <Badge variant="destructive"><XCircle className="w-3 h-3 mr-1" />거절됨</Badge>
      case 'COMPLETED':
        return <Badge className="bg-blue-100 text-blue-700"><CheckCircle className="w-3 h-3 mr-1" />완료</Badge>
      default:
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-4 mt-3">
              <span className="text-sm">
                <strong>{user.followerCount?.toLocaleString() || 0}</strong> 팔로워
              </span>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{app.campaign?.business?.name}</p>
        ...
        <TabsContent value="liked" className="mt-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {/* 무한 스크롤 센티널 */}
            {likeHasMore && (
              <div ref={likeSentinelRef} className="col-span-full h-10 flex justify-center">
                {likeLoading && (
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" />
                )}
              </div>
