...
async function analyzeIndexes() {
  ...
  try {
    ...
    // campaigns 테이블
    const hasCampaignStatusIndex = campaignIndexes.some((idx: any) => 
    ...
    const hasCategoryCompositeIndex = categoryIndexes.some((idx: any) =>
    ...
    const hasApplicationStatusIndex = applicationIndexes.some((idx: any) =>
      idx.indexdef.includes('status') && idx.indexdef.includes('deletedAt')
    );
    ...
    tableStats.forEach((stat: any) => {
      const deadRatio = stat.row_count > 0 
        ? ((stat.dead_rows / stat.row_count) * 100).toFixed(2)
        : '0';
      console.log(`  - ${stat.tablename}: ${stat.row_count} rows (${deadRatio}% dead)`);
